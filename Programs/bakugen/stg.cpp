...
void ShDraw(){
	...
	for(list<SHOTDATA>::iterator it=Shot.begin();it!=Shot.end();++it){
		if((*it).late>0){
			...
			float large = 1.0 + 2.0 * ((float)(*it).late/(float)(*it).max_late);
			(*it).late--;
			SetDrawBlendMode(DX_BLENDMODE_ADD,(*it).flash);
			if(repeat==9)	DrawRotaGraphF((*it).x+shake_x,(*it).y+shake_y,large,Radian((*it).angle,TRUE),(*it).img,TRUE,FALSE);
		}
		else{
			ShMove(it);
