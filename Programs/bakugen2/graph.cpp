...
int gPlayer[PLAYER_GRAPH_NUM][PLAYER_COLOR_NUM][PLAYER_CHIP_NUM];
...
int GLoad(){
	try{
		...
		char fi[128];
		SetTransColor(0,0,0);
		int gra[PLAYER_GRAPH_NUM][PLAYER_COLOR_NUM];
		for(int i=0;i<PLAYER_GRAPH_NUM;i++){
			for(int j=0;j<PLAYER_COLOR_NUM;j++){
				wsprintf(fi , "data/img/dot/%d-%d.png" , i ,j);
				if( (gra[i][j] = LoadGraph(fi)) == -1)	throw(fi);
			}
		}
		for(int i=0;i<PLAYER_GRAPH_NUM;i++){
			for(int j=0;j<PLAYER_COLOR_NUM;j++){
				for(int a=0;a<PLAYER_CHIP_NUM;a++){
					gPlayer[i][j][a] = DerivationGraph(24*(a%3), 32*(a/3), 24, 32, gra[i][j]);
				}
			}
		}
		...
		for(int i=0;i<FACE_GRAPH_NUM;i++){
			wsprintf(fi , "data/img/face/%d.png" , i);
		...
		for(int i=0;i<SHOT_GRAPH_NUMA;i++){
			for(int a=0;a<SHOT_GRAPH_NUMB;a++){
				...
				if(i==2)	gShot[i][a] = DerivationGraph(30*a, 0, 30, 30, sgra[i]);
			}
		}
		...
		for(int i=0;i<EFFECT_GRAPH_NUM;i++){
			...
			if(i==6||i==7){
				SetMaskTransColor(GetColor(255,0,0));
				if( (gEffect[i] = LoadMask(fi)) == -1)	throw(fi);
			}
			else{
