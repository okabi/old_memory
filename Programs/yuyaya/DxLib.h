...
// ＤＸライブラリのバージョン
#define DXLIB_VERSION 0x308e
#define DXLIB_VERSION_STR _T( "3.08e" )
...
extern	int			GetPcInfo(						TCHAR *OSString , TCHAR *DirectXString , TCHAR *CPUString , int *CPUSpeed /* 単位MHz */ , double *FreeMemorySize /* 単位MByte */ , double *TotalMemorySize , TCHAR *VideoDriverFileName , TCHAR *VideoDriverString , double *FreeVideoMemorySize /* 単位MByte */ , double *TotalVideoMemorySize ) ;	// ＰＣの情報を得る
...
extern	int			SetWindowIconHandle(					HICON Icon ) ;																		// 使用するアイコンのハンドルをセットする
extern	int			SetUseASyncChangeWindowModeFunction(	int Flag , void (* CallBackFunction )( void * ) , void *Data ) ;					// 最大化ボタンやALT+ENTERキーによる非同期なウインドウモードの変更の機能の設定を行う
...
extern	int			SetWindowMinimizeFlag(					int Flag ) ;																		// メインウインドウを最小化するかどうかのフラグをセットする
...
// ツールバー関係
extern	int			SetupToolBar(				const TCHAR *BitmapName , int DivNum , int ResourceID = -1 ) ;															// ツールバーの準備( BitmapName に NULL, ResourceID に -1 を指定するとツールバーを解除、ResourceID が -1 以外で BitmapName が NULL の時にのみ ResourceID が使用される )
...
extern	int			AddMenuItem(				int AddType /* MENUITEM_ADD_CHILD等 */ , const TCHAR *ItemName, int ItemID, int SeparatorFlag, const TCHAR *NewItemName = NULL , int NewItemID = -1 ) ;	// メニューに項目を追加する
...
// グラフィックハンドル作成関係関数
extern	int			MakeGraph(						int SizeX, int SizeY, int NotUse3DFlag = FALSE ) ;							// グラフィックハンドルを作成する
...
extern	int			FillGraph(						int GrHandle, int Red, int Green, int Blue, int Alpha = 255 ) ;				// グラフィックハンドルを特定の色で塗りつぶす
extern	int			SetGraphLostFlag(				int GrHandle, int *LostFlag ) ;												// グラフィックハンドル削除時に立てるフラグのアドレスをセットする
extern	int			InitGraph(						int LogOutFlag = FALSE ) ;													// すべてのグラフィックハンドルを削除する
extern	int			ReloadFileGraphAll(				void ) ;																	// ファイルから読み込んだ画像をグラフィックハンドルに再度読み込む
...
extern	int			GetCreateGraphColorBitDepth(				void ) ;										// 作成するグラフィックの色深度を取得する
extern	int			SetCreateGraphChannelBitDepth(				int BitDepth ) ;								// 作成するグラフィックの１チャンネル辺りのビット深度を設定する
extern	int			GetCreateGraphChannelBitDepth(				void ) ;										// 作成するグラフィックの１チャンネル辺りのビット深度を取得する
...
extern	int			SetDrawValidGraphCreateZBufferFlag(			int Flag ) ;									// 描画可能画像を作成する際に専用のＺバッファも作成するかどうかを設定する( デフォルトでは TRUE( 作成する ) )
extern	int			GetDrawValidGraphCreateZBufferFlag(			void ) ;										// 描画可能画像を作成する際に専用のＺバッファも作成するかどうかを取得する
...
extern	int			GetMaxGraphTextureSize(			int *SizeX, int *SizeY ) ;																// グラフィックに使用される最大テクスチャサイズを取得する
...
extern	int			DrawRotaGraph3(           int x, int y, int cx, int cy, double ExtRateX, double ExtRateY, double Angle, int GrHandle, int TransFlag, int TurnFlag = FALSE ) ; 				// グラフィックの回転描画３
...
extern	int			DrawRotaGraph3F(          float xf, float yf, float cxf, float cyf, double ExtRateX, double ExtRateY, double Angle, int GrHandle, int TransFlag, int TurnFlag = FALSE ) ; 	// グラフィックの回転描画３
...
extern	int			DrawChipMap(              int Sx, int Sy, int XNum, int YNum, int *MapData, int ChipTypeNum, int MapDataPitch, int *ChipGrHandle, int TransFlag ) ;																											// チップグラフィックを使ったマップ描画
extern	int			DrawChipMap(              int MapWidth, int MapHeight,        int *MapData, int ChipTypeNum,                   int *ChipGrHandle, int TransFlag, int MapDrawPointX, int MapDrawPointY, int MapDrawWidth, int MapDrawHeight, int ScreenX, int ScreenY ) ;	// チップグラフィックを使ったマップ描画
extern	int			DrawTile(                 int x1, int y1, int x2, int y2, int Tx, int Ty, double ExtRate, double Angle, int GrHandle, int TransFlag ) ;																														// グラフィックを指定領域にタイル状に描画する
...
extern	VECTOR		ConvWorldPosToViewPos(				VECTOR WorldPos ) ;												// ワールド座標をカメラ座標に変換する
...
extern	int				BltDrawValidGraph(							int TargetDrawValidGrHandle, int x1, int y1, int x2, int y2, int DestX, int DestY, int DestGrHandle ) ;							// 描画可能グラフィックハンドルから指定領域の画像情報をグラフィックハンドルに転送する
...
extern	int				GetDrawScreen(								void ) ;																		// 描画先画面の取得
...
extern	int			SetNotUse3DFlag(							int Flag ) ;									// ( 同効果のSetUseSoftwareRenderModeFlag を使用して下さい )３Ｄ機能を使わないフラグのセット、効果は SetUse3DFlag と同じ
extern	int			SetUse3DFlag(								int Flag ) ;									// ( 同効果のSetUseSoftwareRenderModeFlag を使用して下さい )３Ｄ機能を使うか、のフラグをセット
extern	int			GetUse3DFlag(								void ) ;										// 描画に３Ｄ機能を使うかフラグを取得
extern	int			SetScreenMemToVramFlag(						int Flag ) ;									// ( 同効果のSetUseSoftwareRenderModeFlag を使用して下さい )画面データをＶＲＡＭに置くか、フラグ
...
#ifdef __WINDOWS__
extern	int			SetUseDirectDrawFlag(						int Flag ) ;									// ( 同効果のSetUseSoftwareRenderModeFlag を使用して下さい )ＤｉｒｅｃｔＤｒａｗを使用するかどうかをセットする
extern	int			SetUseGDIFlag(								int Flag ) ;									// ＧＤＩ描画を必要とするか、を変更する
extern	int			SetDDrawUseGuid(							GUID *Guid ) ;									// ＤｉｒｅｃｔＤｒａｗが使用するＧＵＤＩを設定する
extern	void*		GetUseDDrawObj(								void ) ;										// 現在使用しているＤｉｒｅｃｔＤｒａｗオブジェクトのアドレスを取得する( 戻り値を IDirectDraw7 * にキャストして下さい )
...
extern	int			GetUseGDIFlag(								void ) ;										// ＧＤＩ描画を必要とするかの状態を取得する
extern	DX_DIRECT3DDEVICE9* GetUseDirect3DDevice9(				void ) ;										// 使用中のＤｉｒｅｃｔ３ＤＤｅｖｉｃｅ９オブジェクトを得る
#endif // __WINDOWS__
extern	int			RefreshDxLibDirect3DSetting(				void ) ;										// ＤＸライブラリのＤｉｒｅｃｔ３Ｄ設定をしなおす( 特殊用途 )
extern	int			RenderVertex(								void ) ;										// 頂点バッファに溜まった頂点データを吐き出す( 特殊用途 )
...
extern	int			CalcPolygonIndexedBinormalAndTangentsToShader( VERTEX3DSHADER *Vertex, int VertexNum, unsigned short *Indices, int PolygonNum ) ;		// ポリゴンの頂点の接線と従法線をＵＶ座標から計算してセットする(インデックス)
...
extern	int			FontBaseImageBlt(                       int x, int y, const TCHAR *StrData, BASEIMAGE *DestImage, BASEIMAGE *DestEdgeImage,                 int VerticalFlag = FALSE ) ;															// 基本イメージに文字列イメージを転送する
extern	int			FontBaseImageBltToHandle(               int x, int y, const TCHAR *StrData, BASEIMAGE *DestImage, BASEIMAGE *DestEdgeImage, int FontHandle, int VerticalFlag = FALSE ) ;															// 基本イメージに文字列イメージを転送する
...
extern	int			CreateGraphImage_plus_Alpha(   const TCHAR *FileName, void *RgbImage, int RgbImageSize, int RgbImageType, void *AlphaImage, int AlphaImageSize, int AlphaImageType, BASEIMAGE *RgbGraphImage, BASEIMAGE *AlphaGraphImage, int ReverseFlag ) ;		// 各種グラフィックデータからグラフィックイメージデータとアルファマップ用イメージデータを構築する
...
#ifndef DX_NON_FONT
extern	int			BltStringSoftImage(              int x, int y, const TCHAR *StrData, int DestSIHandle, int DestEdgeSIHandle = -1 ,                                        int VerticalFlag = FALSE ) ;		// ソフトウエアで扱うイメージに文字列イメージを転送する
extern	int			BltStringSoftImageToHandle(      int x, int y, const TCHAR *StrData, int DestSIHandle, int DestEdgeSIHandle /* 縁が必要ない場合は -1 */ , int FontHandle, int VerticalFlag = FALSE ) ;		// ソフトウエアで扱うイメージに文字列イメージを転送する( フォントハンドル使用版 )
#endif // DX_NON_FONT
